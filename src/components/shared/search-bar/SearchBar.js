...
const SearchBar = () => {
	...
	const onFilterUsers = (e) => {
        ...
        newArray = users.filter(item => item.username.toLowerCase().includes(searchTerm));
    ...
    return (
        <ContentWrapper>
        <form className="form">
            <input className="input" type="search" onChange={onFilterUsers} placeholder="Search in Let's Vest..." spellCheck="false" />
            <button>
